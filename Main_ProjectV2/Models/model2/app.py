...
class ECGAnalysis(db.Model):
    ...
    user_id = db.Column(db.String(255), nullable=False, default="anonymous_user")  # Default user ID
    timestamp = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)
    report = db.Column(db.Text, nullable=False, default="No analysis available yet.")  # Default report
    document_base64 = db.Column(db.Text, nullable=False, default="")  # Default empty document
...
load_dotenv("api.env")
api_key = "AIzaSyB_iSnnpCEbqb2VsjEPsC-VKpu62VFMLqI"
if api_key is None:
...
def get_analysis_history():
    user_id = request.args.get('user_id')
    print(user_id)
    x = ECGAnalysis.query.all()
    for i in x:
        print(i.user_id)
    if not user_id:
        return jsonify({'error': 'User ID is required'}), 400
    try:
        analyses = ECGAnalysis.query.filter_by(user_id=user_id).order_by(ECGAnalysis.timestamp.desc()).all()
        history = [{
            'id': analysis.id,
